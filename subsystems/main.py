    ...
    while True:
        ...
        try:   # §10.3: Exception handling — loop never crashes demo
            ...
            for name, sub in subs.items():
                ...
                # ── Fault Isolation — ALL active faults this tick ─
                # Use isolate_all() so simultaneous faults (e.g. overcurrent
                # + battery_drain) each get their own pipeline run (§10.2)
                diagnoses = sub["isolator"].isolate_all(h)
                if not diagnoses:
