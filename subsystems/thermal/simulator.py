...
class ThermalSubsystemSimulator:
    ...
    def _apply_active_faults(self):
        for fault_type, fault in list(self.active_faults.items()):
            ...
            if fault_type == "thermal_runaway":
                self.internal_temp = NOMINAL["internal_temp"]["nominal"] + t * (target - NOMINAL["internal_temp"]["nominal"])
                if self.internal_temp > 55:
                    self.radiator_temp = min(100.0, self.radiator_temp + random.uniform(0.1, 0.4))
            elif fault_type == "heater_failure":
                self.heater_power = NOMINAL["heater_power"]["nominal"] + t * (target - NOMINAL["heater_power"]["nominal"])
                if self.heater_power < 25:
                    self.panel_temp = max(0.0, self.panel_temp - random.uniform(0.15, 0.45))
            elif fault_type == "radiator_degradation":
                self.radiator_temp = NOMINAL["radiator_temp"]["nominal"] + t * (target - NOMINAL["radiator_temp"]["nominal"])
                if self.radiator_temp > 40:
                    self.internal_temp = min(100.0, self.internal_temp + random.uniform(0.05, 0.2))

    def _apply_recovery_drift(self):
        rate = random.uniform(0.08, 0.16)
        heater_target = NOMINAL["heater_power"]["nominal"] * 0.85 if self.backup_heater_active else NOMINAL["heater_power"]["nominal"]
    ...
    def tick(self, power_current: float = None) -> dict:
        ...
        if power_current is not None and power_current > 35:
            excess_heat = (power_current - 35) * 0.02   # reduced: background warming only
